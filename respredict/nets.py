...
class GraphVertModel(nn.Module):
    ...
    def forward(self, adj, vect_feat, input_mask, input_idx, 
...
class NoUncertainLoss(nn.Module):
    ...
    def __call__(self, pred, y,  mask, vert_mask):
        ...
        y_masked = y[mask>0].reshape(-1, 1) * self.scale
...
class SGCModel(nn.Module):
    def __init__(self, g_feature_n=4,
                 int_d = 128, OUT_DIM = 1,
        ...
        self.gml = GraphMatLayerFastSCM(g_feature_n, int_d, GS=GS,
                                        agg_func = parse_agg_func(agg_func), 
                                        nonlin=gml_nonlin)
        ...
        if not resnet_out:
            ...
            self.lin_out_first = ResNetRegression(int_d,
                                               block_sizes = resnet_blocks, 
                                               INT_D = resnet_d, 
                                               FINAL_D=resnet_d, 
                                               OUT_DIM=int_d)
...
class SGCModelNoAgg(nn.Module):
    def __init__(self, g_feature_n=4,
        ...
        self.gml = GraphMatLayerFast(g_feature_n, int_d, GS=GS,
                                     agg_func = None)
...
class PredVertOnly(nn.Module):
    def __init__(self, g_feature_n=4,
        ...
        if not resnet_out:
            ...
            if use_batchnorm:
                self.lin_out_first = ResNetRegressionMaskedBN(int_d,
                                                              block_sizes = resnet_blocks, 
                                                              INT_D = resnet_d, 
                                                              FINAL_D=resnet_d, 
                                                              OUT_DIM=int_d)
    ...
    def forward(self, adj, vect_feat, input_mask, input_idx, 
        ...
        if self.resnet_out:
            if self.use_batchnorm:
                x = F.relu(self.lin_out_first(g_squeeze_flat, input_mask_flat).reshape(BATCH_N, MAX_N, -1))
...
class RelNetFromS2S(nn.Module):
    def __init__(self, vert_f_in, edge_f_in, MAX_N, 
        ...
        self.lin_e_layers = nn.ModuleList([nn.Linear(internal_d_vert, 
                                                     internal_d_vert, 
                                                     bias=layer_use_bias)
                                        for _ in range(self.layer_n)])

        self.lin_v_layers = nn.ModuleList([nn.Linear(internal_d_vert, 
                                                  internal_d_vert, 
                                                     bias=layer_use_bias)
                                        for _ in range(self.layer_n)])
        ...
        if self.bilinear_vv:
            self.lin_vv_layers = nn.ModuleList([nn.Bilinear(internal_d_vert, 
                                                            internal_d_vert, 
                                                            internal_d_vert, 
                                                            bias=layer_use_bias)
                                                for _ in range(self.layer_n)])
        elif self.gru_vv:
            self.lin_vv_layers = nn.ModuleList([nn.GRUCell(internal_d_vert, 
                                                           internal_d_vert, 
                                                           bias=layer_use_bias)
                                                for _ in range(self.layer_n)])
        else:
            self.lin_vv_layers = nn.ModuleList([nn.Linear(internal_d_vert +  internal_d_vert, 
                                                          internal_d_vert, 
                                                          bias=layer_use_bias)
                                                for _ in range(self.layer_n)])
            

        self.bn_v_layers = nn.ModuleList([nn.BatchNorm1d(internal_d_vert)
                                        for _ in range(self.layer_n)])
        
        self.bn_e_layers = nn.ModuleList([nn.BatchNorm1d(internal_d_vert)
                                        for _ in range(self.layer_n)])
        ...
        if not resnet_out:
            ...
            self.lin_out = ResNetRegression(internal_d_vert,
                                            block_sizes = resnet_blocks, 
                                            INT_D = resnet_d, 
                                            FINAL_D=final_d_out, 
                                            OUT_DIM=final_d_out)
...
class GraphVertModelExtraVertInput(nn.Module):
    def __init__(self, g_feature_n, g_feature_out_n=None, 
        ...
        if not resnet_out:
            ...
            self.lin_out = ResNetRegression(g_feature_out_n[-1] + extra_vert_in_d, 
                                            block_sizes = resnet_blocks, 
                                            INT_D = resnet_d, 
                                            FINAL_D=resnet_d, 
                                            OUT_DIM=OUT_DIM)
...
class GraphVertModelMaskedBN2(nn.Module):
    def __init__(self, g_feature_n, g_feature_out_n=None, 
        ...
        if not resnet_out:
            ...
            self.lin_out_mu = ResNetRegression(g_feature_out_n[-1], 
                                            block_sizes = resnet_blocks, 
                                            INT_D = resnet_d, 
                                            FINAL_D=resnet_d, 
                                            OUT_DIM=OUT_DIM)
...
class GraphWithUncertainty(nn.Module):
    def __init__(self, g_feature_n, g_feature_out_n=None, 
                 int_d = None, common_layer_n = None,
                 split_layer_n = 2, 
                 resnet=True, 
                 gml_class = 'GraphMatLayers',
                 gml_config = {}, 
        ...
        self.gml_shared = eval(gml_class)(g_feature_n, g_feature_out_n, 
                                   resnet=resnet, noise=init_noise,
                                   agg_func=parse_agg_func(agg_func), 
                                   norm = inner_norm, 
                                   GS=GS,
        ...
        self.gml_mean = eval(gml_class)(g_feature_out_n[-1], split_f,
                                        resnet=resnet, noise=init_noise,
                                        agg_func=parse_agg_func(agg_func), 
                                        norm = inner_norm, 
                                        GS=GS,
                                        **gml_config)
        self.gml_var = eval(gml_class)(g_feature_out_n[-1], split_f,
                                       resnet=resnet, noise=init_noise,
                                       agg_func=parse_agg_func(agg_func), 
                                       norm = inner_norm, 
                                       GS=GS,
        ...
        self.res_mean = ResNetRegressionMaskedBN(split_f[-1], 
                                                 block_sizes = resnet_blocks, 
                                                 INT_D = resnet_d, 
                                                 FINAL_D=resnet_d, 
                                                 OUT_DIM=OUT_DIM)
        self.res_var = ResNetRegressionMaskedBN(split_f[-1], 
                                                 block_sizes = resnet_blocks, 
                                                 INT_D = resnet_d, 
                                                 FINAL_D=resnet_d, 
                                                 OUT_DIM=OUT_DIM)
    ...
    def forward(self, adj, vect_feat, input_mask, input_idx, adj_oh,
        ...
        g_mean_squeeze_flat = G_mean_features.squeeze(1).reshape(-1, G_mean_features.shape[-1])
        g_var_squeeze_flat = G_var_features.squeeze(1).reshape(-1, G_var_features.shape[-1])

        
        x_mean = self.res_mean(g_mean_squeeze_flat, input_mask.reshape(-1)).reshape(BATCH_N, MAX_N, -1)
        x_var = self.res_var(g_var_squeeze_flat*self.output_std_scale, input_mask.reshape(-1)).reshape(BATCH_N, MAX_N, -1)
        ...
        return {'mu' : x_mean * self.output_scale,
...
class GraphVertConfig(nn.Module):
    def __init__(self, g_feature_n, g_feature_out_n=None, 
        ...
        if not resnet_out:
            ...
            self.lin_out = ResNetRegressionMaskedBN(g_feature_out_n[-1], 
                                                    block_sizes = resnet_blocks, 
                                                    INT_D = resnet_d, 
                                                    FINAL_D=resnet_d,
                                                    norm = resnet_norm,
    ...
    def forward(self, adj, vect_feat, input_mask, input_idx, adj_oh,
        ...
        if self.resnet_out:
            x_1 =  self.lin_out(g_squeeze_flat, input_mask.reshape(-1)).reshape(BATCH_N, MAX_N, -1)
        else:
            x_1 = self.lin_out(g_squeeze)
